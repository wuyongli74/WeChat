...
1.获取用户的收货地址
  ...
  3.获取用户对小程序所授予获取地址的权限状态scope
    ...
    2.假设用户点击获取收货地址的提示框'取消' 
      scope值为false
      1.诱导用户自己打开授权页面(wx.openSetting)，当用户重新给与获取地址权限的时候
      2.获取收货地址
...
Page({
  ...
  onShow() {
    ...
    // 1.计算全选
    // every 数组方法会遍历，会接收一个回调函数，那么每一个回调函数都返回true；那么every方法的返回值为true
  ...
  // 设置购物车状态，同时重新计算底部工具栏的数据，全选、总价格、购买数量
  setCart(cart) {
    let allChecked = true
    // 1.总价格和总数量
    let totalPrice = 0
